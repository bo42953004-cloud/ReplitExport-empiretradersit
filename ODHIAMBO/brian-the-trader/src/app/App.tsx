        ...
        const handleCallback = async () => {
            try {
                ...
                if (accounts && accounts.length > 0) {
                    ...
                    // Prefer the first real account; fall back to first account overall
                    const realAccount = accounts.find(
                        a => !a.account_id.startsWith('VRT') && !a.account_id.startsWith('VRTC')
                    );
                    const firstAccount = realAccount || accounts[0];
                    ...
                    // Clean the OAuth params from the URL then do a fresh page load.
                    // A clean reload is more reliable than trying to re-initialise the
                    // WebSocket in-place: it guarantees the app starts with all stored
                    // auth data (auth_info + active_loginid + deriv_accounts) already
                    // in localStorage and picks up the OTP-authenticated WebSocket URL
                    // through the normal api_base.init() boot path.
                    window.location.replace(window.location.origin);
                } else {
                    console.error('[Auth] No accounts returned after authentication');
