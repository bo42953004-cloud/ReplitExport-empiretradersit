...
export default Engine =>
    class Proposal extends Engine {
        makeProposals(trade_option) {
            // Fast mode: force 1-tick contracts so the bot trades on every tick.
            // Multiplier and Accumulator contracts do not use duration, so skip them.
            if (window.__empireSpeedMode === 'fast') {
                const skipTypes = ['MULTUP', 'MULTDOWN', 'ACCU'];
                const needsOverride = !(trade_option.contractTypes || []).some(t => skipTypes.includes(t));
                if (needsOverride) {
                    trade_option = { ...trade_option, duration: 1, duration_unit: 't' };
                }
            }
