...
class APIBase {
    ...
    /**
     * Handles real-time balance update messages pushed by Deriv's WebSocket.
     * Updates the authData$ and account_list$ observables so the header balance
     * and account-switcher dropdown reflect live changes without a page refresh.
     */
    private handleBalanceUpdate(balanceData: { balance: number; currency: string; loginid: string }) {
        const { balance, currency, loginid } = balanceData;

        // Update authData$ when the active account's balance changes
        const currentAuthData = authData$.getValue();
        if (currentAuthData && loginid === currentAuthData.loginid) {
            setAuthData({ ...currentAuthData, balance, currency });
        }

        // Update every account in the list so the switcher dropdown stays current
        const currentList = account_list$.getValue();
        if (currentList?.length) {
            const updatedList = currentList.map(acc =>
                acc.loginid === loginid ? { ...acc, balance } : acc
            );
            setAccountList(updatedList);
        }

        // Keep the MobX client store in sync (drives legacy balance displays)
        const clientStore = globalObserver.getState('client.store');
        if (clientStore && loginid === getAccountId()) {
    ...
    async subscribe() {
        ...
        // Subscribe to all incoming WebSocket messages and handle balance updates
        // in real-time so the header reflects balance changes the moment they occur.
        this.api?.onMessage().subscribe({
            next: (message: any) => {
