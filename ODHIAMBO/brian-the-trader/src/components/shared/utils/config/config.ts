...
export const generateOAuthURL = async (prompt?: string): Promise<string> => {
    try {
        ...
        // Use the env-configured redirect URI when available (must match exactly
        // what is registered in the Deriv OAuth app settings).  Falls back to
        // window.location.origin so local dev still works without the var set.
        const redirectUri = process.env.NEXT_PUBLIC_REDIRECT_URI || window.location.origin;
        console.log('[OAuth] clientId:', clientId);
        console.log('[OAuth] redirectUri being sent to Deriv:', redirectUri);
