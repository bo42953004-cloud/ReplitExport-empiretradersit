...
import SocialLinksButton from '@/components/social-links-button/SocialLinksButton';
...
const AppHeader = observer(() => {
    ...
    useEffect(() => {
        ...
        if (activeLoginid) { setIsOAuthPending(false); return; }
    ...
    useEffect(() => {
        ...
        const timer = setTimeout(() => {
            if (isAuthorizing && !activeLoginid) { setAuthTimeout(true); setIsAuthorizing(false); }
        }, 5000);
        if (activeLoginid || !isAuthorizing) { if (authTimeout) setAuthTimeout(false); clearTimeout(timer); }
        return () => clearTimeout(timer);
    ...
    const handleSignup = useCallback(async () => {
        try {
            ...
            else { console.error('Failed to generate OAuth URL for signup'); setIsAuthorizing(false); }
        } catch (error) { console.error('Signup redirection failed:', error); setIsAuthorizing(false); }
    }, [setIsAuthorizing]);
    ...
    const handleLogin = useCallback(async () => {
        try {
            ...
            if (oauthUrl) window.location.replace(oauthUrl);
            else { console.error('Failed to generate OAuth URL'); setIsAuthorizing(false); }
        } catch (error) { console.error('Login redirection failed:', error); setIsAuthorizing(false); }
    }, [setIsAuthorizing]);

    const handleTransfer = useCallback(() => {
        const transferCurrency = authData?.currency;
        if (!transferCurrency) { console.error('No currency available for transfer'); return; }
    ...
    const renderAccountSection = useCallback(
        (position: 'left' | 'right' = 'right') => {
            if (activeLoginid && !is_account_regenerating) {
                if (position === 'left' && !isDesktop) {
                    ...
                    return (
                        <div className='auth-actions'>
                            ...
                            <Button primary disabled={client?.is_logging_out || !authData?.currency} onClick={handleTransfer}>
                ...
                return (
                    <div className='auth-actions'>
                        <Button tertiary disabled={!isAuthConfigured} onClick={handleLogin}><Localize i18n_default_text='Log in' /></Button>
                        <Button primary_light disabled={!isAuthConfigured} onClick={handleSignup}><Localize i18n_default_text='Sign up' /></Button>
                    </div>
                );
            } else if (position === 'right') {
                return (
        ...
        <>
            <Header className={clsx('app-header', { 'app-header--desktop': isDesktop, 'app-header--mobile': !isDesktop })}>
                <Wrapper variant='left'>
                    ...
                    <AppLogo />
                    {/* Social links button — always visible next to logo */}
                    <SocialLinksButton />
