...
export default class RunPanelStore {
    ...
    onRunButtonClick = async () => {
        // Apply the speed mode the user last selected in the UI toggle.
        // Proposal.js reads this to decide whether to force 1-tick contracts.
        try {
            const stored = localStorage.getItem('trade_speed_mode');
            (window as any).__empireSpeedMode = stored === 'fast' ? 'fast' : 'normal';
        } catch {
            (window as any).__empireSpeedMode = 'normal';
        }
