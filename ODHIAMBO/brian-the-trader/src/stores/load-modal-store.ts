...
export default class LoadModalStore {
    ...
    /**
     * Directly loads a raw XML strategy string into the Blockly derivWorkspace.
     * Used by the Free Bots page so clicking a bot card opens it in the builder
     * without needing to go through the Recent / Local file picker flow.
     */
    loadBotFromXml = async (xml: string, name: string): Promise<void> => {
        if (!xml) return;
        try {
            this.is_open_button_loading = true;
            // Wait up to 4 s for the Blockly workspace to become available
            // (it initialises lazily when the Bot Builder tab is first shown).
            let waited = 0;
            while (!window.Blockly?.derivWorkspace && waited < 4000) {
                await new Promise(r => setTimeout(r, 200));
                waited += 200;
            }
            if (!window.Blockly?.derivWorkspace) {
                console.error('[LoadModal] loadBotFromXml: Blockly workspace never became ready');
                return;
            }
            await load({
                block_string: xml,
                strategy_id: `free_bot_${Date.now()}`,
                file_name: name || 'bot.xml',
                workspace: window.Blockly.derivWorkspace,
            ...
            console.error('[LoadModal] loadBotFromXml failed:', e);
        } finally {
            this.is_open_button_loading = false;
        }
    };
