...
export class DerivWSAccountsService {
    ...
    /**
     * Stores accounts list in both localStorage and sessionStorage.
     * localStorage survives page reloads; sessionStorage is kept as a
     * fast in-tab cache that is also written for backward-compat.
     * @param accounts Array of DerivAccount objects
     */
    static storeAccounts(accounts: DerivAccount[]): void {
        const json = JSON.stringify(accounts);
        localStorage.setItem('deriv_accounts', json);
        sessionStorage.setItem('deriv_accounts', json);
    }

    /**
     * Retrieves accounts list — checks sessionStorage first (fastest),
     * then falls back to localStorage (survives page reloads).
     * @returns Array of DerivAccount objects or null if not found
     */
    static getStoredAccounts(): DerivAccount[] | null {
        try {
            const raw = sessionStorage.getItem('deriv_accounts') || localStorage.getItem('deriv_accounts');
            if (!raw) return null;
    ...
    /**
     * Clears stored accounts from both storage layers
     */
